     <div id="rightcontent">
        ...
        <div id="searchForm">
            <form name="search_ads" method="POST" action="midroll.php?m=midroll">
            <table width="100%" cellpadding="0" cellspacing="5" border="0">
            <tr>
                <td align="right">Sort</td>
                <td>
                    <select name="sort">
                    <option value="title"{if $option.sort == 'added'} selected="selected"{/if}>Title</option>
		    ...
		    <input type="submit" name="search_ads" value=" -- Search -- " class="button">
                    <input type="reset" name="reset_search" value=" -- Clear -- " class="button">
                </td>
            </tr>
	    <tr>
		<td colspan="6" align="left" valign="bottom">
		    <input type="submit" name="new_ad" value=" -- Create New Midroll -- " class="button" style="margin-left:15px;">
		</td>
	    </tr>
            </table>
            </form>
		...
		<td align="center"><b>Views/Hits</b></td>
            ...
            <tr bgcolor="{cycle values="#F8F8F8,#F2F2F2"}">
                ...
                <td align="left">{$ad.title|escape:'html'}</td>
		<td align="center">{if $ad.url != ''}<a href="{$ad.url}" target="_blank">{$ad.url|truncate:80}</a>{/if}</td>
                <td align="center"><b>{$channels[$ad.channel]}</b></td>
		...
		<td align="center">{$ad.expire|date_format}</td>
                <td align="center">
                    <a href="nuevo.php?m=midrolledit&ID={$ad.ID}">Edit</a><br>
                    <a href="nuevo.php?m=midroll&a=delete&ID={$ad.ID}" onClick="javascript:return confirm('Are you sure you want to delete this midroll?');">Delete</a><br>
                    {if $ad.active == '1'}
                    <a href="nuevo.php?m=midroll&a=suspend&ID={$ad.ID}">Suspend</a>
                    {else}
                    <a href="nuevo.php?m=midroll&a=activate&ID={$ad.ID}">Activate</a>
                    {/if}
                    <br>
                </td>
            </tr>
