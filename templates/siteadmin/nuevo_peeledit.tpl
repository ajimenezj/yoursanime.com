     <div id="rightcontent">
        ...
        <h2>Peel Effect</h2>
        <div id="simpleForm">
        <form name="edit_ad" method="POST" action="nuevo.php?m=peeledit&ID={$data.ID}">
        <fieldset style="font-size:11px">
        <legend>Edit peel effect</legend>
	    ...
	    <label for="target">Window target: </label>
            <select name="target">
	    <option value="_blank" selected="selected">New windows</option>
	    <option value="_self"{if $data.target == "_self"} selected="selected"{/if}>Same window</option>
	    </select>
	    <br />
	...
	<div style="text-align: center;">
	    <input type="submit" name="submit" value="Update Peel Effect" class="button">
        </div>
        </form>
        </div>
        </div>
